        ...
        value: function findElement(path) {
          ...
          path.forEach(function (dom, key) {
            if (dom.getAttribute && dom.getAttribute('name') === 'move-block') {
              ...
              _this2.currentDom.setAttribute(
                'class',
                _this2.currentBaseClass + ' ' + _this2.activeClass
              );
            }
          });
        }
      ...
      },
      {
        ...
        value: function setMovePosition(start, end) {
          ...
          this.currentDom.setAttribute(
            'style',
            '\n      position:absolute;\n      top:0;\n      left:0;\n      transform:translate('
              .concat(translate.x + moveX, 'px, ')
              .concat(translate.y + moveY, 'px);\n      z-index: 100;\n    ')
          );
        }
        /*
         * 设置开始移动doms位置
      ...
      },
      {
        ...
        value: function cacheChangePositions(currentKey, key, positions) {
          ...
          moveInfo.push({
            from: currentKey,
            to: key,
            translate: _objectSpread2(
              {},
              this.domToPosition(positions[currentKey], positions[key])
            ),
            moving: _objectSpread2(
              {},
              positions[currentKey].moving || positions[currentKey].translate
            )
          });
